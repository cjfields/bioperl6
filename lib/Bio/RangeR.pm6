# may move common subsets to a base class
#our subset Strand of Int where { $^a == -1 | 0 | 1 };
...
#submethod BUILD (Int :$start?, Int :$end?, :$strand? where { -1 <= $strand <= 1}) {
	#if $start.defined && $end.defined && $start > $end {
	#	
	#}
	#
#}
...
# the below could techinically go into a separate Utils class (not specific to
# one Range, but applicable to a set of ranges)
...
#enum StrandOptions <strong weak ignore>;
...
#	self!length = $len if $len.defined && $len ~~ Int;
#	if self!length.defined  {
#		return self!length;
#	} else {
#		die "Must define both start and end" if !self.start.defined && self.end.defined;
#		die "End must be larger than start" if self.start > self.end;
